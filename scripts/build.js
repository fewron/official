    ...
    try {
        ...
        for (const query of SEARCH_QUERIES) {
            ...
            for (const place of searchRes.data.results) {
                ...
                // 3. 詳細情報を取得（Place Details）
                const details = await axios.get(`https://maps.googleapis.com/maps/api/place/details/json`, {
                    params: {
                        place_id: place.place_id,
                        fields: 'name,formatted_address,formatted_phone_number,place_id,types,website',
                        key: API_KEY,
                        language: 'ja'
                    }
                });
                ...
                // CSVにはどの検索でヒットしたかも残すと管理しやすい
                const csvLine = `"${data.name}","${data.formatted_phone_number || 'N/A'}","${targetUrl}","${query}"\n`;

                if (!await fs.pathExists(CSV_PATH)) {
                    await fs.writeFile(CSV_PATH, '店名,電話番号,発行URL,取得元エリア業種\n');
                }
                await fs.appendFile(CSV_PATH, csvLine);

                console.log(`✅ 新規追加: ${data.name} (from ${query})`);
            }
        }

        await fs.writeJson(JSON_PATH, savedPlaces, { spaces: 2 });
        console.log('--- 全ての検索と生成が完了しました ---');
